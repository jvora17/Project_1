...
%   z3=nodes(bnodes(3),3);
  ...
  bn1 =  [  0.250  0.000  -0.750  0.500];
  bn1d =  [0.75000  0.00000  -0.75000];
  bn1dd =  [   1.500  0.00];
  bn2 =  [ 0.250  -0.250  -0.250  0.250];
  bn2d =  [0.75000  -0.50000  -0.25000];
  bn2dd =  [   1.500  -0.500];
  bn3 =  [-0.250  0.000  0.750  0.500];
  bn3d = [-0.75000  0.00000  0.75000];
  bn3dd =[  -1.5000  0];
  bn4 =  [ 0.2500  0.2500  -0.2500  -0.2500];
  bn4d = [ 0.7500  0.5000  -0.2500];
  bn4dd =[   1.5000  0.5000];
...
%   rn3d=[-2 0];
  ...
  %Euler bernoulli beams must be slender. Warn if not. |sqrt(A3*4/pi)/l>DoverL
  ...
  % This took some work, but provide bounds on other values.|(Izz3+Iyy3)<(1/2.1*A3^2/pi) 
  ...
  slenderness=min([sqrt((Izz1+Iyy1)/A1) ... 
	sqrt((Izz2+Iyy2)/A2)  ])/l;% sqrt((Izz3+Iyy3)/A3)
...
%              polyval(bn6dd,bgpts(i))/Jac
    Izz=polyval(rn1*Izz1+rn2*Izz2,bgpts(i));%Find Izz at +rn3*Izz3
                                                     %Gauss point
...
%               polyval(bn5dd,bgpts(i))/Jac^2;
%              -polyval(bn6dd,bgpts(i))/Jac
    Iyy=polyval(rn1*Iyy1+rn2*Iyy2,bgpts(i));%+rn3*Iyy3
...
%       end
    ...
    J=polyval(rn1*J1+rn2*J2,rgpts(i));% J at gauss point.+rn3*J3
    A=polyval(rn1*A1+rn2*A2,rgpts(i));% A at gauss point+rn3*A3
  ...
  %
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  numbeamgauss=numbeamgauss+2; %Need more gauss points for the mass
                               %matrix. 
  [bgpts,bgpw]=gauss(numbeamgauss);
  ...
  for i=1:numbeamgauss
    ...
    A=polyval(rn1*A1+rn2*A2,bgpts(i));%+rn3*A3
  ...
  for i=1:numrodgauss
    rodsfs=[polyval(rn1,rgpts(i));
            polyval(rn2,rgpts(i))];
            %polyval(rn3,rgpts(i))
    J=polyval(rn1*(Iyy1+Izz1)+rn2*(Iyy2+Izz2),rgpts(i));%+rn3*(Iyy3+Izz3)
    A=polyval(rn1*A1+rn2*A2,rgpts(i));%+rn3*A3
...
% $$$          z z z z z lamloc];
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %
  % Assembling matrices into global matrices bn3=bnodes(3);  bn3*6+(-5:0)
  %
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
